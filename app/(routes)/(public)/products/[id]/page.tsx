...
import { ShoppingCart, Minus, Plus, Check, Tag, Zap, ChevronLeft, ChevronRight } from "lucide-react"
import { useFetchProductByIdQuery } from "@/lib/service/modules/productService"
import type { ProductDetailVariant } from "@/lib/service/modules/productService/type"
...
export default function ProductDetailPage() {
  ...
  const { data: productData, isLoading, error } = useFetchProductByIdQuery(productId)

  const cartItems = useSelector((state: RootState) => {
    const items = (state as any).cart?.data?.items
    return Array.isArray(items) ? items : []
  })

  const getCurrentCartQuantity = (variantId: number) => {
    const existingItem = cartItems.find((item: any) => item.productDetailId === variantId)
  ...
  const capacityOptions = useMemo(() => {
    ...
    productData.data.variants
      .filter((variant: ProductDetailVariant) => variant.color?.id === selectedVariant.color?.id)
  ...
  const uniqueImagesByColor = useMemo(() => {
    ...
    productData.data.variants.forEach((variant: ProductDetailVariant) => {
      if (variant.color && variant.color.status === "ACTIVE" && variant.thumbnailImage) {
  ...
  const handleColorChange = (colorId: number) => {
    const newVariant = productData?.data?.variants.find(
      (v: ProductDetailVariant) => v.color?.id === colorId && v.status === "ACTIVE"
    )
    if (newVariant) {
  ...
  const handleQuantityChange = (newQuantity: number) => {
    if (selectedVariant) {
      ...
      if (newQuantity >= 1 && newQuantity <= maxQuantity) {
        ...
        toast.warning(`Chỉ có thể thêm tối đa ${maxQuantity} sản phẩm nữa vào giỏ hàng`)
      }
    }
  }
  ...
  const handleAddToCart = async () => {
    ...
    if (quantity > maxQuantity) {
      toast.error(`Số lượng vượt quá giới hạn. Chỉ có thể thêm tối đa ${maxQuantity} sản phẩm`)
      return
    }
    try {
      ...
      toast.error(error?.message || "Có lỗi xảy ra khi thêm sản phẩm vào giỏ hàng")
    }
  }
  ...
  const maxThumbnailIndex = Math.max(0, uniqueImagesByColor.length - thumbnailsPerView)
  ...
  const selectedVariantInfo = selectedVariant ? getPromotionInfo(selectedVariant) : null
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-6 space-y-6">
            <div className="aspect-square relative bg-gray-50 rounded-lg overflow-hidden border">
              <Image
                src={selectedVariant?.thumbnailImage || "/placeholder.svg?height=500&width=500"}
                alt={product.name}
                fill
                className="object-cover"
                priority
              />
              {selectedVariantInfo?.hasPromotion && selectedVariantInfo.promotion && (
                <div className="absolute top-4 left-4">
                  <div className="bg-red-500 text-white px-3 py-1 rounded-md text-sm font-medium">
                    {selectedVariantInfo.promotion.discountType === "percentage"
                      ? `Giảm ${selectedVariantInfo.promotion.discountValue}%`
                      : `Giảm ${formatPrice(selectedVariantInfo.promotion.discountValue || 0)}`}
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="relative">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2 flex-1 overflow-hidden">
                    {visibleThumbnails.map((imageData, index) => (
                      <div
                        key={imageData.colorId}
                        className={`w-16 h-16 flex-shrink-0 relative bg-gray-50 rounded-lg overflow-hidden cursor-pointer border-2 transition-all ${
                      ...
                      >
                        <Image
                          src={imageData.image || "/placeholder.svg?height=64&width=64"}
            ...
            {product.features && product.features.length > 0 && (
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-200">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-lg font-semibold text-blue-900">Tính năng nổi bật</h3>
                </div>
            ...
            {product.tags && product.tags.length > 0 && (
              <div className="p-4 rounded-lg border border-green-200">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-lg font-semibold text-green-900">Tags sản phẩm</h3>
                </div>
          ...
          <div className="lg:col-span-6 space-y-6">
            <div>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-blue-800 font-medium">{product.brand?.name}</span>
                <span className="text-sm text-gray-600">|</span>
                <span className="text-sm text-gray-600">Danh mục:</span>
                <span className="text-purple-800 font-medium">{product.category?.name}</span>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="mb-2 p-2 bg-red-50 rounded-md border border-red-200">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-red-800">
                      {selectedVariantInfo.promotion.title || selectedVariantInfo.promotion.name}
                    </span>
                  </div>
                  <div className="text-xs text-red-600 mt-1">
                    {selectedVariantInfo.promotion.discountType === "percentage"
                      ? `Giảm ${selectedVariantInfo.promotion.discountValue}% - Tiết kiệm ${formatPrice(selectedVariantInfo.discountAmount)}`
                      : `Giảm ${formatPrice(selectedVariantInfo.promotion.discountValue)} - Tiết kiệm ${formatPrice(selectedVariantInfo.discountAmount)}`}
                  </div>
                </div>
              )}
              ...
              {product.minPrice !== product.maxPrice && (
                <div className="text-sm text-gray-600">
                  Giá từ {formatPrice(product.minPriceDiscount)} - {formatPrice(product.maxPrice)}
                </div>
              )}
            </div>
            ...
            {colorOptions.length > 0 && (
              <div>
                <h3 className="text-base font-semibold mb-3 text-gray-900">Màu sắc:</h3>
                <div className="flex flex-wrap gap-2">
            ...
            {capacityOptions.length > 0 && (
              <div>
                <h3 className="text-base font-semibold mb-3 text-gray-900">Dung tích:</h3>
                <div className="flex flex-wrap gap-2">
                  {capacityOptions.map((capacity: any) => (
                    <button
                      key={capacity.id}
                      className={`px-4 py-2 rounded-lg border text-sm font-medium transition-all ${
            ...
            {selectedVariant && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-base font-semibold text-gray-900">Số lượng:</h3>
                  ...
                  <div className="flex items-center gap-4 mb-3 md:mb-0 md:w-1/2">
                    ...
                    <span className="text-sm text-gray-600 whitespace-nowrap">
                      {getCurrentCartQuantity(selectedVariant.id) > 0 &&
                        `(${getCurrentCartQuantity(selectedVariant.id)} trong giỏ hàng)`}
                    </span>
                  </div>
                  ...
                  <div className="md:flex-1">
                    ...
                    >
                      ...
                      {selectedVariant?.availableStock === 0
                        ? "Hết hàng"
                        : `Thêm vào giỏ hàng | ${formatPrice((selectedVariantInfo?.discountedPrice || selectedVariant?.price || 0) * quantity)}`}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3 text-sm">
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    ...
                    <span className="font-medium">{product.material?.name || "Chưa cập nhật"}</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    <span className="text-gray-600">Loại vải, thành phần chính của sản phẩm:</span>
                  ...
                  <div className="flex justify-between py-2 border-b border-gray-100">
                    ...
                    <span className="font-medium">{product.strapType?.name || "Chưa cập nhật"}</span>
                  </div>
                  {product.waterproofRating && (
                    <div className="flex justify-between py-2 border-b border-gray-100">
                      <span className="text-gray-600">Độ chống nước:</span>
                      <span className="font-medium">{product.waterproofRating}</span>
                    </div>
                  )}
                </div>
              </div>
