...
const getFilterIds = (
  ...
  const plural = searchParams.getAll(pluralKey).map(Number).filter(Boolean);
  const singular = searchParams.get(singularKey);
  const singularId = singular ? Number(singular) : null;
  if (singularId && !isNaN(singularId)) {
    return [...new Set([...plural, singularId])];
  }
  return plural;
};
...
export default function ProductsPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    brandIds: getFilterIds(searchParams, "brandIds", "brandId"),
    categoryIds: getFilterIds(searchParams, "categoryIds", "categoryId"),
    colorIds: getFilterIds(searchParams, "colorIds", "colorId"),
    materialIds: getFilterIds(searchParams, "materialIds", "materialId"),
    strapTypeIds: getFilterIds(searchParams, "strapTypeIds", "strapTypeId"),
    capacityIds: getFilterIds(searchParams, "capacityIds", "capacityId"),
    tagIds: getFilterIds(searchParams, "tagIds", "tagId"),
    featureIds: getFilterIds(searchParams, "featureIds", "featureId"),
    ...
    maxPrice: searchParams.get("maxPrice")
      ...
      : undefined,
    hasPromotion:
      searchParams.get("hasPromotion") === "true" ? true : undefined,
  });
  ...
  const { data: productsData } = useFetchProductsQuery({
    ...
    materialIds:
      filters.materialIds.length > 0 ? filters.materialIds : undefined,
    strapTypeIds:
      filters.strapTypeIds.length > 0 ? filters.strapTypeIds : undefined,
    capacityIds:
      filters.capacityIds.length > 0 ? filters.capacityIds : undefined,
  ...
  const updateFilters = (newFilters: Partial<typeof filters>) => {
    ...
    updatedFilters.brandIds?.forEach((id) =>
      params.append("brandIds", String(id))
    );
    updatedFilters.categoryIds?.forEach((id) =>
      params.append("categoryIds", String(id))
    );
    updatedFilters.colorIds?.forEach((id) =>
      params.append("colorIds", String(id))
    );
    updatedFilters.materialIds?.forEach((id) =>
      params.append("materialIds", String(id))
    );
    updatedFilters.strapTypeIds?.forEach((id) =>
      params.append("strapTypeIds", String(id))
    );
    updatedFilters.capacityIds?.forEach((id) =>
      params.append("capacityIds", String(id))
    );
    updatedFilters.tagIds?.forEach((id) => params.append("tagIds", String(id)));
    updatedFilters.featureIds?.forEach((id) =>
      params.append("featureIds", String(id))
    );
        ...
        {!isCollapsed && (
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {items?.map((item) => {
              ...
              const isChecked =
                (filters[type] as number[])?.includes(id) || false;
              return (
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateFilters({
                            hasPromotion: checked ? true : undefined,
                          })
                        }
                      />
                      <Label
                  ...
                  {productsData?.pagination ? (
                    <>
                      ...
                      <span className="text-[#FF6B00]">
                        {productsData.pagination.totalElements}
                      </span>{" "}
                      sản phẩm
                    </>
                  ) : (
